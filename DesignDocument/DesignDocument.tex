...
\newglossaryentry{neural network}
{
	name=neural network,
	description={
	The neural network itself is not an algorithm, but rather a framework for many different machine learning algorithm to work together and process complex data inputs.
...
\newglossaryentry{default neural network}
{
	...
	description={The default neural network is the network we will be using if the user does not specify a different architecture. It will be a Convolutional \gls{neural network}}
}
\newglossaryentry{CLI}
{
	name=command line interface,
	description={The terminal box where the user can enter his commands for the interaction with the program}
}
...
\newglossaryentry{exception}
{
	name=exception,
	plural=exceptions,
	description={An exception is an failure that may occur in a programs life cycle due to unwanted user input or system behavior}
}

\newglossaryentry{HDF5}
{
	name=HDF5,
	description={HDF5 is a file format that can be used for storing and organizing big amounts of data like many big matrices}
}
...
\begin{figure}[h]
\begin{center}
\includepdf[width=\textwidth, height= \textheight, keepaspectratio]{ClassDiagrams/PDF/Big_classdiagram.pdf}
\label{Class Diagram}
\end{center}
\end{figure}
%include big class diagramm

We will be structuring our programm in five main kategories: The colletor, the labling module, the training module, the classifier and the command line interface. They will be explained in detail in the following chapters.
...
\begin{figure}[h]
\begin{center}
\includepdf[width=\textwidth, height= \textheight, keepaspectratio]{SequenceDiagram/PDF/Collector_collect_sequenzdiagramm.pdf}
