...
\item We decided to cut the possibility to select a density, because we couldn't guarantee that the fetched and cut matrices from Suite Sparse have the wanted density.
...
When the Suite Sparse collection is extended the list can be updated to include the new matrices. 
...
The smallest share of matrices of a given entity must be no less than 30\% of the total number of contained matrices.
...
\item A implemented and trained \gls{neural network} of the \gls{ResNet} architecture. 
...
Our main problem was with \gls{Ginkgo}. 
...
It worked with a remote interpreter.
